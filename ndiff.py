...
class Scan(object):
    ...
    def to_dom_fragment(self, document):
        ...
        elem = document.createElement(u"scan")
        if self.scanner is not None:
            elem.setAttribute(u"scanner", self.scanner)
        if self.args is not None:
            elem.setAttribute(u"args", self.args)
        if self.start_date is not None:
            elem.setAttribute(u"start", "%d" % timegm(self.start_date.utctimetuple()))
...
class ScanDiff(object):
    ...
    def to_dom_fragment(self, document):
        frag = document.createDocumentFragment()
        a_elem = document.createElement(u"a")
        a_elem.appendChild(self.scan_a.to_dom_fragment(document))
        frag.appendChild(a_elem)
        b_elem = document.createElement(u"b")
        b_elem.appendChild(self.scan_b.to_dom_fragment(document))
    ...
    def print_xml(self, f = sys.stdout):
        ...
        # scan run changes
        scandiff_elem.appendChild(self.to_dom_fragment(document))
